...
export default function Header() {
	...
	const stepInfo = useMemo(() => [ 'Select a box', 'Select desired MTXs', 'Simulate Boxes', 'Global Stats' ], []);
	...
	return (
		<div className="header">
			...
			<h1>MTX Calc</h1>
			<h2>
				Step {currStep + 1}:<span>{stepInfo[currStep]}</span>
			</h2>
			<div
				className={`forwardStep ${onCheckStepVisibilityForward(currStep)}`}
				onClick={() => onStepButton(currStep + 1)}
			>
				<i className="fas fa-arrow-circle-right" />
				<div>
					<h3>
						Step{' '}
						{!activeMTX.length && !allowStepThree ? (
							2
						) : currStep === 1 && allowStepThree ? (
							3
						) : currStep < 2 ? (
							currStep + 2
						) : currStep === 2 && !simList ? (
