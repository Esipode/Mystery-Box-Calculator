	...
	return (
		<div className="boxSimulatorContainer">
			<div className="boxSimToggle">
				<p
					className={`toggleContainer ${simMode === 'simulator' ? 'active' : ''} ${simRunning
						? 'disable-btn'
						: ''}`}
					onClick={() => !simRunning && setSimMode('simulator')}
				>
					<i className="fas fa-box-open" />Simulator
				</p>
				<p
					className={`toggleContainer ${simMode === 'probability' ? 'active' : ''} ${simRunning
						? 'disable-btn'
						: ''}`}
					onClick={() => !simRunning && setSimMode('probability')}
				>
					<i className="fas fa-percentage" />Probability
				</p>
			</div>
