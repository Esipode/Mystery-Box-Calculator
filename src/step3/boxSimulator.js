...
export default function BoxSimulator({simToggle, isRunning, curMTXList, fullMTXList, setStatList, boxChanged, safariCheck}) {
	...
	return (
		<div style={{minWidth: '100vw'}}>
			<SimBoxes safariCheck={safariCheck} setStatList={setStatList} setSubmitted={setSubmitted} setResultsPending={setResultsPending} isRunning={isRunning} onSubmitResults={onSubmitResults} completedList={completedList} boxChanged={boxChanged} resultsSubmitted={resultsSubmitted} setBoxVal={setBoxVal} boxVal={boxVal} simToggle={simToggle} resultsPending={resultsPending} fullMTXList={fullMTXList} />
		</div>
	);
