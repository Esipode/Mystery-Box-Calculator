...
export default function ProbabilitySim({ simMode, sleep }) {
	...
	const getBoxImage = () =>
		activeBox && box.filter((boxName) => boxName.name === activeBox && boxName.image)[0].image;
	...
	const calcBox = async () => {
		...
		for (let i = 0; i < iterations; ) {
			...
			if (!simRunning) {
				...
				if (
					selectList.filter((mtx) => mtx.count >= 1).length === selectList.length ||
					...
					newList.unshift({
						...
						points: currVals.boxes * 30,
						found: [ selectList.filter((mtx) => mtx.count >= 1).length, selectList.length ],
						percent: (selectList.filter((mtx) => mtx.count >= 1).length * 100 / currVals.boxes || 0)
							.toFixed(2)
					});
	...
	const calcItem = async () => {
		...
		for (let i = 0; i < iterations; ) {
			...
			if (!simRunning) {
				...
				if (selectItem.count || (parseInt(boxVal) >= 1 && currVals.boxes === parseInt(boxVal))) {
		...
		<div className={`probabilitySim${simMode === 'probability' ? '' : ' hideContainer'}`}>
			{activeBox && (
				<div className="selectorContainer">
					<h4
						className={`boxSelector ${probFilter === 'box' ? 'activeFilter' : ''} ${simRunning
							? 'disabledContainer'
							: ''}`}
					...
					<Select
						className={`react-select ${probFilter === 'item' ? 'activeSelect' : ''} ${simRunning
							? 'disabledContainer'
							: ''}`}
			...
			<div className="input-container">
				<h4 className={`iteration-input ${simRunning ? 'disabled' : ''}`}>
					...
					<input
						...
						maxLength="2"
						disabled={simRunning}
						onChange={(e) => setIterations(e.target.value > 20 ? 20 : e.target.value.replace(/\D/, ''))}
						value={iterations}
						placeholder="#"
										...
										<td>
											{round.found ? (
												<i
													...
													style={{ color: '#090', textShadow: '0px 0px 2px white' }}
												/>
											) : (
												<i
													className="fas fa-times"
													style={{ color: '#b00', textShadow: '0px 0px 2px white' }}
												/>
											)}
										</td>
										<td>{round.percent} %</td>
